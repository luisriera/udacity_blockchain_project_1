...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            block.previousBlockHash = chainHeight === -1 ? null : self.chain[chainHeight].hash;
            block.time = new Date().getTime().toString().slice(0,-3)
            block.hash = await SHA256(JSON.stringify(block)).toString();
            block.height = chainHeight + 1;

            let num_elements = self.chain.push(block) - 1;
            this.height = num_elements;

            num_elements > -1 ? resolve(block) : reject(new Error("No able to append block to the chain!"));
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(currentTime - messageTime > 300){
                reject(new Error("Elapsed time greater than 5 min!"))
            }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const block_found = self.chain.filter(p => p.hash === hash)[0];
            block_found !== null ? resolve(block_found) : reject(new Error("No a block with hash: " + hash ))
        });
    }
